...
class DeclarativeFieldsMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        attrs['base_fields'] = dict([(field_name, attrs.pop(
            field_name)) for field_name, obj in list(attrs.items()) if isinstance(obj, Field)])
    ...
    def __init__(self, data={}, **kwargs):
        ...
        for name, value in list(kwargs.items()):
            if name not in self.fields:
                raise TypeError(
                    "__init__() got an unexpected keyword argument '%s'" % name)
